...
public class SensorDataHandlerToFile implements SensorDataHandler {
    ...
    //3
    @Override
    public void receiveData(Collection<SensorQueue> data) {// After the 2 queses are sent then this function starts from here
        Log.d(TAG, "Received " + data.size() + " queues");

        for (SensorQueue queue : data) {//running 2 times for 2 queues
            ...
            Log.d("******Sensor Type: ", String.valueOf(sensor));
            FileHandler handler = mFileHandlers.get(1);
            try {
                if (handler == null) {
                    ...
                    mFileHandlers.put(1, handler);
                }
                handler.writeQueue(queue,sensor);
            }
            catch (IOException e) {
